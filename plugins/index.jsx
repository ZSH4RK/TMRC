...
async function autoLaunch() {
    // Find the game (TmForever)
    let gameName = loader.config.default_game;
    if (!gameName) {
        // Fall back to first game in database
        let entries = Object.entries(loader.database.values);
        if (entries.length > 0) {
            gameName = entries[0][0];
        }
    }
    if (!gameName) {
        log("Auto-launch: no game found");
        return;
    }
    let game = loader.database.get_game(gameName);
    if (!game) {
        log("Auto-launch: game '" + gameName + "' not found");
        return;
    }
    // Find the default profile
    let profileName = (loader.config.default_profiles && loader.config.default_profiles[gameName]) || "default";
    let profile = game.get_profile(profileName);
    if (!profile) {
        log("Auto-launch: profile '" + profileName + "' not found, trying first profile");
        let entries = Object.entries(game.profiles.values);
        if (entries.length > 0) {
            profile = entries[0][1];
            profileName = entries[0][0];
        }
    }
    if (!profile) {
        log("Auto-launch: no profile found");
        return;
    }
    log("Auto-launch: starting profile '" + profileName + "'");
    let proc = profile.prepare();
    if (!proc) {
        log("Auto-launch: prepare() returned null");
        return;
    }
    await proc.async_start();
    log("Auto-launch: launch complete");
}
...
export class UiGames extends Element {
    ...
    static get() {
        return document.getElementById("_games");
    }
    render() {
    ...
    UxGame(name, game, selected = false) {
        if (selected) {
            this.set(game);
        }
    ...
    set(game = null) {
        let same = game?.get_name() == this.game?.get_name();
        this.game = game;
        if (!same) {
...
export class UiProfiles extends Element {
    ...
    static get() {
        return document.getElementById("_profiles");
    }
    render() {
        let game = UiGames.get().game;
        if (!game) {
    ...
    UxProfile(name, prof, selected = false) {
        if (selected) {
            this.set(prof);
        }
        let fixed = prof.get_name();
        return <button class={selected ? "panel selected" : "panel"} id={"_prof_" + fixed} title={fixed} onclick={() => { this.set(prof); }}><div class="panel-header">
...
export class UiProducts extends Element {
    ...
    static get() {
        return document.getElementById("_products");
    }
    render() {
    ...
    UxProd(name, prod, selected = false, filled = false) {
        let clazz = "panel";
        if (selected) {
            clazz += " selected";
        }
        return <button class={clazz} id={"_prod_" + name} title={name} onclick={() => {
                this.set(prod);
            }}><div class="panel-header">
...
export class UiDescription extends Element {
    ...
    static get() {
        return document.getElementById("_description");
    }
    render() {
        let profile = UiProfiles.get().profile;
        ...
        <button class="panel">
          <div class="panel-header">
            ...
            <div class="panel-caption"><input class="panel-input" type="text" title="Edit name" id="_edit_profile" value={profile.get_name()} onkeydown={(e) => {
        ...
        <button class="panel" title="Args"><div class="panel-header"><div class="panel-caption"><b>Args:</b>
          <input type="text" class="panel-input" id="_args" oninput={() => {
                    let value = document.getElementById("_args")?.value;
...
Admin privileges might be required.</div>
    ...
    UxSelector(profile, selector, is_mod = true) {
        if (selector) {
          ...
          <div class={selector.version ? "panel-description" : "panel-description collapsed"} id={"_desc_sel_" + selector.id}><b>Version:</b>
            <input type="text" class="panel-input" id={"_sel_ver_" + selector.id} value={selector.version} oninput={() => {
                    let value = document.getElementById("_sel_ver_" + selector.id)?.value;
                    if (value && value.length) {
    ...
    UxVersion(name, version, collapsed = false, filled = false) {
        ...
        <div class="panel-caption"><b>{name}</b></div>
        <div class="panel-buttons">{version.is_installed() ?
                <button class="panel-button" role="panel-remove" title="Uninstall" onclick={async () => {
...
export class UiConfig extends Element {
    ...
    static get() {
        return document.getElementById("_config");
    }
    render() {
        return <span>
...
export class UiLog extends Element {
    ...
    static get() {
        return document.getElementById("_log");
    }
    componentDidMount() {
        this.content(this.render());
    }
